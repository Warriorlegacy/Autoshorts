...
exports.getProviders = exports.regenerateVideo = exports.deleteVideo = exports.getUserVideos = exports.getVideoStatus = exports.getVideo = exports.generateVideo = void 0;
...
const scriptService = new scriptService_1.ScriptService();
const renderingService = new renderingService_1.RenderingService();
const ttsService = (0, ttsService_1.getTTSService)();
const imageService = (0, imageService_1.getImageGenerationService)();
const generateVideo = async (req, res) => {
    try {
        const { niche, duration, visualStyle, language, voiceName, speakingRate, generateImages, scriptProvider, scriptModel, imageProvider, imageModel, ttsProvider, ttsModel } = req.body;
        ...
        let scenesWithAudio = await Promise.all(script.scenes.map(async (scene, index) => {
            try {
        ...
        if (generateImages) {
            ...
            scenesWithAudio = await Promise.all(scenesWithAudio.map(async (scene) => {
                try {
        ...
        let result;
        try {
            result = await (0, db_1.query)(`INSERT INTO videos (id, user_id, title, caption, niche, language, duration, visual_style, status, scenes, metadata)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
const getVideo = async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)('SELECT * FROM videos WHERE id = ? AND user_id = ?', [videoId, userId]);
...
const getVideoStatus = async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)('SELECT id, title, caption, status, video_url, thumbnail_url, created_at FROM videos WHERE id = ? AND user_id = ?', [videoId, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Video not found' });
        }
        const video = result.rows[0];
...
const deleteVideo = async (req, res) => {
    try {
        ...
        const checkResult = await (0, db_1.query)('SELECT id FROM videos WHERE id = ? AND user_id = ?', [videoId, userId]);
        if (checkResult.rows.length === 0) {
            return res.status(404).json({ message: 'Video not found' });
        }
        // Delete video
        await (0, db_1.query)('DELETE FROM videos WHERE id = ? AND user_id = ?', [videoId, userId]);
        res.status(200).json({
            success: true,
...
const regenerateVideo = async (req, res) => {
    try {
        const { videoId } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const videoResult = await (0, db_1.query)(`SELECT id, title, caption, niche, duration, visual_style, language, metadata, scenes 
       FROM videos WHERE id = ? AND user_id = ?`, [videoId, userId]);
        ...
        if (hasImages) {
            ...
            scenesWithAudio = await Promise.all(scenesWithAudio.map(async (scene) => {
                try {
                    ...
                    const imageResult = await imageService.generateImage({
                        prompt: imagePrompt,
                        style: video.visual_style || 'cinematic',
                        aspectRatio: '9:16',
                        quality: 'high',
                        provider: video.imageProvider,
                        model: video.imageModel,
                    });
                    return {
                        ...scene,
                        background: {
        ...
        await (0, db_1.query)(`UPDATE videos 
...
exports.regenerateVideo = regenerateVideo;
/**
