import { API_KEYS, SCRIPT_PROVIDERS } from '../constants/config';
...
export class TopicScriptService {
   ...
   */
  async generateScript(options: ScriptGenerationOptions): Promise<GeneratedScript | null> {
    if (!this.isAvailable()) {
...
Output ONLY valid JSON in the exact format specified.`
   ...
   */
  private buildScriptPrompt(options: ScriptGenerationOptions): string {
...
Context:
- Niche: ${niche || 'General'}
- Tone: ${tone || 'educational'} - ${toneStyles[tone as keyof typeof toneStyles] || toneStyles.educational}
- Length: ${lengthWords[length as keyof typeof lengthWords] || lengthWords.medium}
...
Return ONLY this JSON format (no markdown, no explanation):
{
   ...
   */
  async getSuggestedTopics(niche: string, count: number = 5): Promise<string[]> {
    if (!this.isAvailable()) {
...
Example: ["Topic 1", "Topic 2", "Topic 3"]`
   ...
   */
  getDefaultTopics(niche: string): string[] {
...
// Singleton instance
let topicScriptService: TopicScriptService | null = null;

export function getTopicScriptService(): TopicScriptService {
