...
class InstagramService {
   ...
   */
  async getInstagramAccountId(accessToken: string, pageId: string): Promise<InstagramAccountResponse> {
    const response = await fetch(
      `${FACEBOOK_GRAPH_API}/${pageId}?fields=instagram_business_account&access_token=${accessToken}`
    );
    ...
    return response.json() as Promise<InstagramAccountResponse>;
  }

  /**
   * Get user's Facebook Pages
   */
  async getUserPages(accessToken: string): Promise<Array<{ id: string; name: string }>> {
    const response = await fetch(
   ...
   */
  async getInstagramAccountDetails(instagramAccountId: string, accessToken: string): Promise<{ id: string; username: string }> {
    const response = await fetch(
      `${FACEBOOK_GRAPH_API}/${instagramAccountId}?fields=id,username&access_token=${accessToken}`
    );
   ...
   */
  async getStoredAccount(userId: string): Promise<any | null> {
    const result = await query(
      `SELECT id, user_id, platform, platform_user_id, platform_username, access_token, refresh_token, token_expires_at, is_active
    ...
    if (expiresAt.getTime() - now.getTime() < bufferTime) {
      try {
        ...
        await query(
          ...
          [newToken.access_token, newExpiresAt.toISOString(), userId]
        );
  ...
  async uploadReel(
    ...
    try {
      ...
      if (!containerResponse.ok) {
        const errorData = await containerResponse.json() as FacebookErrorResponse;
        throw new Error(`Failed to create reel container: ${errorData.error?.message || containerResponse.statusText}`);
      }

      const containerData = await containerResponse.json() as ReelUploadResponse;
      ...
      if (!publishResponse.ok) {
        const errorData = await publishResponse.json() as FacebookErrorResponse;
        throw new Error(`Failed to publish reel: ${errorData.error?.message || publishResponse.statusText}`);
      }

      const publishData = await publishResponse.json() as ReelPublishResponse;
   ...
   * Check upload status
   */
  async checkUploadStatus(containerId: string, accessToken: string): Promise<UploadStatusResponse> {
    const response = await fetch(
  ...
  async validateToken(accessToken: string): Promise<boolean> {
    try {
      const response = await fetch(
        `${FACEBOOK_GRAPH_API}/debug_token?input_token=${accessToken}&access_token=${this.appId}|${this.appSecret}`
      );
