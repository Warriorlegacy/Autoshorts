...
export interface TTSResponse {
  audioUrl: string;
  audioBuffer: Buffer;
  duration: number; // in milliseconds (approximate)
  isMock: boolean; // Whether this is a mock audio file (should be skipped in rendering)
}
...
export class TextToSpeechService {
   ...
   */
  private async synthesizeWithMurf(options: TTSOptions): Promise<TTSResponse | null> {
    ...
    try {
      ...
      const data = await response.json() as MurfResponse;
      ...
      await promisify(fs.writeFile)(filepath, Buffer.from(audioBuffer));
  ...
  private getMurfVoiceId(languageCode: string, gender: string): string {
   ...
   */
  private async synthesizeWithPythonScript(options: TTSOptions): Promise<TTSResponse> {
    const filename = `audio_${Date.now()}_${Math.random().toString(36).substring(7)}.mp3`;
    const filepath = path.join(this.rendersDir, filename);
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', async (code: number) => {
        ...
        if (fs.existsSync(filepath)) {
          const audioBuffer = await promisify(fs.readFile)(filepath);
   ...
   */
  private async synthesizeWithEdgeTTS(options: TTSOptions): Promise<TTSResponse> {
    const filename = `audio_${Date.now()}_${Math.random().toString(36).substring(7)}.mp3`;
    const filepath = path.join(this.rendersDir, filename);
   ...
   */
  private runEdgeTTSSpawn(pythonPath: string, voice: string, text: string, outputPath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      const { spawn } = require('child_process');
      ...
      proc.on('close', (code: number) => {
        ...
        if (fs.existsSync(outputPath)) {
          ...
          const backendPort = process.env.PORT || '3001';
          resolve(`http://localhost:${backendPort}/renders/${path.basename(outputPath)}`);
  ...
  /**
    ...
    */
  private async executeCommand(command: string, checkFilePath?: string): Promise<void> {
    const pythonPaths = [
      `C:\\Python314\\Scripts`,
      `C:\\Python313\\Scripts`,
      `C:\\Python312\\Scripts`,
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(command, { timeout: 120000, env }, (error, stdout, stderr) => {
   ...
   */
  private findEdgeTTSPath(): string {
   ...
   */
  private buildEdgeTTSCommand(pythonPath: string, voice: string, text: string, outputPath: string): string {
   ...
   */
  private getVoiceForLanguage(languageCode: string, gender: string): string {
   ...
   */
  private async generateSilentAudioWithFfmpeg(options: TTSOptions): Promise<TTSResponse> {
   ...
   * Get Edge TTS voices
   */
  private getEdgeTTSVoices(languageCode?: string): any[] {
    const allVoices = [
      // English (US)
   ...
   */
  async cleanupOldAudio(maxAgeHours = 24): Promise<number> {
    try {
